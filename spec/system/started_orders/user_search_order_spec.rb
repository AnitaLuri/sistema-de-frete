...
describe 'Visitante busca uma ordem de serviço' do
  it 'com sucesso um pedido pendente' do
    ...
    visit root_path
    fill_in 'Buscar Ordem de Serviço', with: order.code
    click_on 'Buscar'

    expect(page).to have_content "Resultado da busca: #{order.code}"
    ...
    expect(page).to have_content 'Pedido ainda não foi iniciado.'
  end
  it 'com sucesso um pedido iniciado' do
    ...
    expect(page).to have_content "Iniciado em: #{I18n.localize(Date.today)}"
  end
end